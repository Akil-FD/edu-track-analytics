...
import {
  LayoutDashboard, BookOpen, BarChart2, LogOut, Menu, X, GraduationCap} from 'lucide-react';
import { ROLES, ROUTES } from '../utils/constants';

const navItems = [
  { to: ROUTES.TEACHER.DASHBOARD, label: 'Dashboard', icon: LayoutDashboard },
  { to: ROUTES.TEACHER.ARTICLES, label: 'My Articles', icon: BookOpen },
  { to: ROUTES.TEACHER.ANALYTICS, label: 'Analytics', icon: BarChart2 },
];
...
export default function TeacherLayout() {
  ...
  useEffect(() => {
    ...
    if (authUser.role !== ROLES.TEACHER) { navigate(ROUTES.TEACHER.DASHBOARD); return; }
  }, [authUser]);

  if (!authUser || authUser.role !== ROLES.TEACHER) return null;
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="lg:hidden flex items-center gap-4 px-4 py-3 bg-white border-b border-slate-200">
          ...
          <span className="text-slate-800 font-semibold">EduLearn — Teacher</span>
        </header>
